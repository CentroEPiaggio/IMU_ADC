...
qb_class_imu::qb_class_imu(){
	...
	if (!imuboard_chain_.empty()){
		...
		imuboard_pub_adc_ = node_->advertise<qb_interface::adcSensorArray>("/qb_class_imu/adc", 1);
		// imuboard_pub_adc_.publish(adc);
		...
		//Adc_.resize(imuboard_chain_[0]->n_imu_,3);
...
bool qb_class_imu::readADC(){

	qb_interface::adcSensor tmp_adc;
	qb_interface::adcSensorArray adc;

	for (int k = imuboard_chain_.size(); k--;){
	//for (int k = 1; k--;){
	    	//if (imuboard_chain_[k]->getADCReadings() < 0)
				//return false;
		imuboard_chain_[k]->getADCReadings();
		//for (int i = 0; i < imuboard_chain_[k]->n_imu_; i++) 
		//for (int i = 0; i < 2; i++) 	
			//{
			//tmp_adc.id = 1;
				// //tmp_adc.id = 0;
		tmp_adc.adc_sensor_1  = imuboard_chain_[k]->adc_raw_[0];
		tmp_adc.adc_sensor_2  = imuboard_chain_[k]->adc_raw_[1];
				//tmp_adc.adc1  = 0;
				//tmp_adc.adc2  = 0;
		adc.m.push_back(tmp_adc);

				//Adc_(i,0) = tmp_adc.adc1; 
				//Adc_(i,1) = tmp_adc.adc2; 
